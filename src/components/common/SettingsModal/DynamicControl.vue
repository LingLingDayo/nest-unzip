...
<template>
  <!-- Slider -->
  <div v-if="config.type === 'slider'" :class="['w-full', config.label ? 'space-y-1' : '']">
    <div class="flex items-center justify-between">
        <label v-if="config.label" :for="config.id" class="block text-xs font-bold tracking-wider text-app-text-dim">{{ config.label }}</label>
        <span class="text-[11px] text-center font-bold font-mono text-app-primary bg-app-primary-light px-2.5 py-0.5 rounded-md border border-app-primary/10 tracking-widest">{{ ctrlValue }}</span>
    </div>
    <p v-if="config.description" class="text-[10px] text-app-text-mute leading-relaxed italic opacity-80">{{ config.description }}</p>
  ...
  <!-- Input -->
  <div v-else-if="config.type === 'input'" :class="['w-full', config.label ? 'space-y-1' : '']">
    <label v-if="config.label" :for="config.id" class="block text-xs font-bold tracking-wider text-app-text-dim">{{ config.label }}</label>
    <p v-if="config.description" class="text-[10px] text-app-text-mute leading-relaxed italic opacity-80">{{ config.description }}</p>
    <div class="relative group flex items-center">
        <input 
          ...
          class="w-full bg-app-surface border border-app-border rounded-lg px-3 py-1.5 text-xs text-app-text font-medium placeholder:text-app-text-mute focus:outline-none focus:border-app-primary/50 focus:ring-4 focus:ring-app-primary/5 transition-all shadow-app-sm"
        ...
        <div 
          ...
          :class="config.inputType === 'number' ? 'right-8' : 'right-3'"
        >
  ...
  <!-- Textarea -->
  <div v-else-if="config.type === 'textarea'" :class="['w-full', config.label ? 'space-y-1' : '']">
    <label v-if="config.label" :for="config.id" class="block text-xs font-bold tracking-wider text-app-text-dim">{{ config.label }}</label>
    <p v-if="config.description" class="text-[10px] text-app-text-mute leading-relaxed italic opacity-80">{{ config.description }}</p>
    <textarea 
      :id="config.id"
      v-model="ctrlValue"
      :rows="config.rows || 3"
      class="w-full bg-app-surface border border-app-border rounded-xl px-3 py-2 text-xs text-app-text font-medium placeholder:text-app-text-mute focus:outline-none focus:border-app-primary/50 focus:ring-4 focus:ring-app-primary/5 transition-all shadow-app-sm resize-y custom-scrollbar leading-relaxed"
      :placeholder="config.placeholder || ''"
    ></textarea>
  </div>
  
  <!-- Select-->
  <div v-else-if="config.type === 'select'" :class="['w-full', config.label ? 'space-y-1' : '']">
    <label v-if="config.label" :for="config.id" class="block text-xs font-bold tracking-wider text-app-text-dim">{{ config.label }}</label>
    <p v-if="config.description" class="text-[10px] text-app-text-mute leading-relaxed italic opacity-80">{{ config.description }}</p>
    <div class="relative">
      <div 
        ...
        class="w-full bg-app-surface border border-app-border rounded-lg px-3 py-1.5 text-xs font-medium shadow-app-sm cursor-pointer transition-all flex items-center justify-between"
        :class="[
      ...
      >
        <div 
          v-if="isSelectOpen"
          class="absolute z-50 w-full bg-app-surface border border-app-border rounded-lg shadow-app-md py-1 max-h-48 overflow-y-auto custom-scrollbar"
          :class="[
        ...
        >
          <div 
            ...
            class="px-3 py-1.5 text-xs font-medium cursor-pointer transition-colors flex items-center justify-between group hover:bg-app-bg"
            :class="[
          ...
          <div v-if="config.type === 'select' && (!config.options || config.options.length === 0)" class="px-3 py-2 text-xs text-app-text-mute text-center italic">
            无可用选项
          </div>
        </div>
      </transition>
  ...
  <!-- Switch -->
  <label v-else-if="config.type === 'switch'" class="flex items-center justify-between cursor-pointer group py-2 px-3 bg-app-bg rounded-xl border border-app-border/40 hover:border-app-primary/30 transition-all duration-500">
    <div class="flex flex-col pr-6">
      <span v-if="config.label" class="text-xs font-bold tracking-wider text-app-text-dim group-hover:text-app-text transition-colors">{{ config.label }}</span>
      <span v-if="config.description" class="text-[10px] text-app-text-mute italic mt-0.5 leading-relaxed opacity-80">{{ config.description }}</span>
    </div>
    <div class="relative shrink-0">
      <input type="checkbox" v-model="ctrlValue" class="sr-only peer">
      <div class="w-9 h-5 bg-app-border rounded-full peer peer-checked:bg-app-primary transition-all after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-4 after:w-4 after:transition-all after:shadow-app-sm peer-checked:after:translate-x-4 animate-all duration-300"></div>
    </div>
  </label>

  <!-- Radio -->
  <div v-else-if="config.type === 'radio'" :class="[config.label ? 'space-y-1' : '']">
    <label v-if="config.label" class="block text-xs font-bold tracking-wider text-app-text-dim">{{ config.label }}</label>
    <p v-if="config.description" class="text-[10px] text-app-text-mute leading-relaxed italic opacity-80">{{ config.description }}</p>
    <div class="flex flex-wrap gap-3">
      <label v-for="opt in config.options" :key="getOptValue(opt)" class="flex items-center space-x-2 cursor-pointer group">
        ...
        <span class="text-[11px] font-bold tracking-wider text-app-text-mute group-hover:text-app-text transition-colors">{{ opt.label }}</span>
      </label>
    </div>
  </div>

  <!-- Button -->
  <div v-else-if="config.type === 'button'" :class="[config.label ? 'space-y-1.5' : '']">
    <div class="flex items-center justify-between">
      <div class="flex flex-col pr-6">
        <span v-if="config.label" class="text-xs font-bold tracking-wider text-app-text-dim">{{ config.label }}</span>
        <span v-if="config.description" class="text-[10px] text-app-text-mute italic mt-0.5 leading-relaxed opacity-80">{{ config.description }}</span>
      </div>
      <button 
        @click="config.type === 'button' && config.onClick?.(settings)"
        class="shrink-0 px-4 py-1.5 font-bold text-xs uppercase tracking-[0.15em] rounded-lg transition-all shadow-app-sm border border-app-border hover:bg-app-primary hover:text-white hover:border-transparent active:scale-95 cursor-pointer"
  ...
  <!-- Checkbox -->
  <div v-else-if="config.type === 'checkbox'" :class="[config.label ? 'space-y-1.5' : '']">
    <label v-if="config.label" class="block text-xs font-bold tracking-wider text-app-text-dim">{{ config.label }}</label>
    <p v-if="config.description" class="text-[10px] text-app-text-mute leading-relaxed italic opacity-80">{{ config.description }}</p>
    ...
    >
      <label 
        ...
        :key="getOptValue(opt)" 
        class="flex items-center space-x-2 px-2 py-1 rounded-lg border transition-all cursor-pointer group shadow-app-sm"
      ...
      >
        ...
        <span class="text-[10px] font-bold tracking-wider text-app-text-mute group-hover:text-app-text truncate transition-colors">{{ opt.label }}</span>
      </label>
    </div>
  ...
  <!-- Info -->
  <div v-else-if="config.type === 'info-text'" :class="[config.label ? 'space-y-1' : '']" class="bg-app-bg/40 p-3 rounded-xl border border-app-border/30">
    <label v-if="config.label" class="block text-xs font-bold tracking-wider text-app-text-dim mb-0.5">{{ config.label }}</label>
    <div class="text-[11px] leading-relaxed text-app-text-mute font-medium">
